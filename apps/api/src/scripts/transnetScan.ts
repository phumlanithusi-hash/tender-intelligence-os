#!/usr/bin/env node
/**
 * Manual Transnet scan command: `pnpm --filter api transnet:scan`.
 * Identical structure/behaviour to eskomScan.ts — see that file's
...
import { TRANSNET_ADAPTER_KEY } from '../lib/adapters/transnet/adapter.js'
...
async function main(): Promise<number> {
  ...
  const adapter = getAdapter(TRANSNET_ADAPTER_KEY)
  if (!adapter) {
    logger.error('The Transnet adapter is not registered — nothing to run.')
    return 1
  }

  const { rows } = await listTenderSources(admin, { limit: 100, offset: 0 })
  const source = rows.find((r) => r.adapter_key === TRANSNET_ADAPTER_KEY)
  if (!source) {
    logger.error(`No tender_sources row has adapter_key = "${TRANSNET_ADAPTER_KEY}" — nothing to scan.`)
    return 1
  }
  ...
  logger.info({ sourceId: source.id, executionId }, 'starting manual Transnet scan')
...
main()
  ...
  .catch((err) => {
    logger.error({ err: err instanceof Error ? err.message : String(err) }, 'Transnet scan crashed')
    process.exit(1)
  })
