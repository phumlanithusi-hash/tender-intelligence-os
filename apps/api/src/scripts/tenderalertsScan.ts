#!/usr/bin/env node
/**
 * Manual TenderAlerts scan command: `pnpm --filter api
 * tenderalerts:scan`. Identical structure/behaviour to
 * eskomScan.ts — see that file's comments for the full rationale.
 * Only the adapter key and log labels differ.
 */
...
async function main(): Promise<number> {
  ...
  const adapter = getAdapter(TENDERALERTS_ADAPTER_KEY)
  if (!adapter) {
    logger.error('The TenderAlerts adapter is not registered — nothing to run.')
    return 1
  }

  const { rows } = await listTenderSources(admin, { limit: 100, offset: 0 })
  const source = rows.find((r) => r.adapter_key === TENDERALERTS_ADAPTER_KEY)
  if (!source) {
    logger.error(`No tender_sources row has adapter_key = "${TENDERALERTS_ADAPTER_KEY}" — nothing to scan.`)
    return 1
  }
...
main()
  ...
  .catch((err) => {
    logger.error({ err: err instanceof Error ? err.message : String(err) }, 'TenderAlerts scan crashed')
    process.exit(1)
  })
