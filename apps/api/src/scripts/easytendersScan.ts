#!/usr/bin/env node
/**
 * Manual EasyTenders scan command: `pnpm --filter api easytenders:scan`.
 * Identical structure/behaviour to etendersScan.ts — see that file's
 * comments for the full rationale (service-role auth, exit codes,
 * health-field updates). Only the adapter key and log labels differ.
 */
...
import { EASYTENDERS_ADAPTER_KEY } from '../lib/adapters/easytenders/adapter.js'
...
async function main(): Promise<number> {
  ...
  const adapter = getAdapter(EASYTENDERS_ADAPTER_KEY)
  if (!adapter) {
    logger.error('The EasyTenders adapter is not registered — nothing to run.')
    return 1
  }

  const { rows } = await listTenderSources(admin, { limit: 100, offset: 0 })
  const source = rows.find((r) => r.adapter_key === EASYTENDERS_ADAPTER_KEY)
  if (!source) {
    logger.error(`No tender_sources row has adapter_key = "${EASYTENDERS_ADAPTER_KEY}" — nothing to scan.`)
    return 1
  }
  ...
  logger.info({ sourceId: source.id, executionId }, 'starting manual EasyTenders scan')
...
main()
  ...
  .catch((err) => {
    logger.error({ err: err instanceof Error ? err.message : String(err) }, 'EasyTenders scan crashed')
    process.exit(1)
  })
