#!/usr/bin/env node
/**
 * Manual City of Cape Town scan command: `pnpm --filter api
 * capetown:scan`. Identical structure/behaviour to etendersScan.ts —
 * see that file's comments for the full rationale.
 */
...
import { CAPETOWN_ADAPTER_KEY } from '../lib/adapters/capetown/adapter.js'
...
async function main(): Promise<number> {
  ...
  const adapter = getAdapter(CAPETOWN_ADAPTER_KEY)
  if (!adapter) {
    logger.error('The City of Cape Town adapter is not registered — nothing to run.')
    return 1
  }

  const { rows } = await listTenderSources(admin, { limit: 100, offset: 0 })
  const source = rows.find((r) => r.adapter_key === CAPETOWN_ADAPTER_KEY)
  if (!source) {
    logger.error(`No tender_sources row has adapter_key = "${CAPETOWN_ADAPTER_KEY}" — nothing to scan.`)
    return 1
  }
  ...
  logger.info({ sourceId: source.id, executionId }, 'starting manual City of Cape Town scan')
  ...
  logger.info(
    ...
    'City of Cape Town scan finished',
  )
...
main()
  ...
  .catch((err) => {
    logger.error({ err: err instanceof Error ? err.message : String(err) }, 'City of Cape Town scan crashed')
    process.exit(1)
  })
