#!/usr/bin/env node
/**
 * Manual Eskom scan command: `pnpm --filter api eskom:scan`. Identical
 * structure/behaviour to etendersScan.ts — see that file's comments
 * for the full rationale. Only the adapter key and log labels differ.
 */
...
import { ESKOM_ADAPTER_KEY } from '../lib/adapters/eskom/adapter.js'
...
async function main(): Promise<number> {
  ...
  const adapter = getAdapter(ESKOM_ADAPTER_KEY)
  if (!adapter) {
    logger.error('The Eskom adapter is not registered — nothing to run.')
    return 1
  }

  const { rows } = await listTenderSources(admin, { limit: 100, offset: 0 })
  const source = rows.find((r) => r.adapter_key === ESKOM_ADAPTER_KEY)
  if (!source) {
    logger.error(`No tender_sources row has adapter_key = "${ESKOM_ADAPTER_KEY}" — nothing to scan.`)
    return 1
  }
  ...
  logger.info({ sourceId: source.id, executionId }, 'starting manual Eskom scan')
...
main()
  ...
  .catch((err) => {
    logger.error({ err: err instanceof Error ? err.message : String(err) }, 'Eskom scan crashed')
    process.exit(1)
  })
