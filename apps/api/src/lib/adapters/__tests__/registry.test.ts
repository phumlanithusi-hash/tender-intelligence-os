...
describe('adapter registry (Phase 4 §6)', () => {
  ...
  it('the production entry point registers exactly eTenders, EasyTenders, and TenderBulletins (the three live sources built so far) — no other adapter yet', async () => {
    __resetAdapterRegistryForTests()
    ...
    expect(listRegisteredAdapterKeys().sort()).toEqual(['easytenders', 'etenders', 'tenderbulletins'])
  })
})
