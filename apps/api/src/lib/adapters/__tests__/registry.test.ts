...
describe('adapter registry (Phase 4 §6)', () => {
  ...
  it('the production entry point registers exactly eTenders, EasyTenders, TenderBulletins, City of Johannesburg, Eskom, City of Cape Town, Transnet, and TenderAlerts (the eight live sources built so far) — no other adapter yet', async () => {
    __resetAdapterRegistryForTests()
    ...
    expect(listRegisteredAdapterKeys().sort()).toEqual(['capetown', 'easytenders', 'eskom', 'etenders', 'joburg', 'tenderalerts', 'tenderbulletins', 'transnet'])
  })
})
