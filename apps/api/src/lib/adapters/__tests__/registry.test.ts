...
describe('adapter registry (Phase 4 §6)', () => {
  ...
  it('the production entry point registers exactly the eTenders adapter (Phase 5 §3) — no other live source yet', async () => {
    __resetAdapterRegistryForTests()
    ...
    expect(listRegisteredAdapterKeys()).toEqual(['etenders'])
  })
})
