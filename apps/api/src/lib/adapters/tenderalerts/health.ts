...
export async function checkTenderAlertsHealth(transport: TenderAlertsTransport): Promise<AdapterHealthCheckResult> {
  ...
  try {
    ...
    return {
      status: 'FAILED',
      message: err instanceof Error ? err.message : 'TenderAlerts health check threw an unexpected error.',
      checkedAt,
    }
  }
}
